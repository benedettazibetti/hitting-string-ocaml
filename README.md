...
Determinare, se esiste, una stringa x di lunghezza N sull’alfabeto {0,1} tale che per
ogni stringa s∈S d(s,x) >0, dove d è la distanza di Hamming.
Si risolva il problema utilizzando una ricerca best first.
Come funzione di valutazione si può il numero stringhe con distanza >0 dalla
soluzione parziale.

##SPIEGAZIONE GENERALE
Sia S un insieme finito e sia x la stringa che voglio trovare, x dev’essere tale che la sua distanza di Hamming sia maggiore di zero da tutti gli elementi di S.
Studio S come se fosse una lista di stringhe con caratteri in Z3. 
Considero un grafo per la ricerca di x costruito tramite funzione successori. Ogni nodo del grafo è una stringa con caratteri in Z2. Ogni successore di x è ottenuto variando un unico carattere della stringa x.

L’esercizio richiede l’utilizzo di una ricerca best first con la funzione di valutazione che conta il numero di elementi dell’iniseme S che hanno distanza di Hamming maggiore di 0 con x.



##SPIEGAZIONE DETTAGLIATA DI TUTTE LE FUNZIONI DEL CODICE
La funzione che definisce la distanza di Hamming è la seguente:
